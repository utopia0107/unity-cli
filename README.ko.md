...
|------|------|
| 0 | 성공 |
| 1 | 명령 실패 — Unity가 실행했지만 에러를 반환 (테스트 실패, 컴파일 에러, 도구 에러) |
| 2 | 사용법 오류 — 잘못된 플래그/인자 (Unity 접속 전) |
| 3 | 연결 실패 — Unity 미실행, 도달 불가, 인스턴스 선택 모호 |
| 4 | CLI와 커넥터의 버전 불일치 |
| 5 | 타임아웃 — 리스너 대기, 컴파일, 테스트 결과 폴링 중 시간 초과 |

결과는 stdout으로, 에러와 진행 메시지는 stderr로 출력됩니다.
