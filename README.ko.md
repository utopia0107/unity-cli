...
이 프로젝트는 정반대의 접근을 택합니다. CLI 전체가 Go ~800줄(+ help text ~300줄), Unity 커넥터가 C# ~1,700줄입니다. 프로토콜 레이어도, 릴레이 프로세스도, 가상 환경도 없습니다 — 바이너리에서 Unity HttpListener로 직접 HTTP POST를 보내는 것이 전부입니다.
